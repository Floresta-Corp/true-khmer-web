...
import { ClipboardList, HandHeart, Target, UsersRound } from "lucide-react";
...
export default function ManagePostCard({ posting, index = 0 }: Props) {
  ...
  const cardHref = `/workspace/volunteer/${posting.id}`;
        ...
        <div className="mt-5" onClick={(e) => e.stopPropagation()}>
          {isCompleted ? (
            ...
            <Button
              ...
              asChild
            >
              <Link to={cardHref}>Review Volunteers</Link>
            </Button>
          )}
        </div>
      </div>
