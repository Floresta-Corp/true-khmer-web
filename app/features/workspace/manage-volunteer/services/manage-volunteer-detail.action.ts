import type { Route } from "project-types/workspace/manage-volunteer/route/+types/manage-volunteer.$id";
import z from "zod";
...
export async function manageVolunteerDetailAction({
  ...
  if (postingAction) {
    ...
    if (!postingActionResult.success || !postingId) {
  ...
  if (!statusActionResult.success || !postingId) {
    return respond({ success: false, error: "Invalid request parameters" });
  }
