  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent
        ...
        className="gap-4 rounded-lg border border-[#e2e8f0] bg-white p-6 shadow-lg min-w-[512px]"
      >
        {/* Close */}
        <DialogClose asChild>
          <Button
            variant="ghost"
            size="icon"
            className="absolute right-[15px] top-[15px] h-4 w-4 rounded-sm p-0 text-[#4a5565]/70 hover:bg-transparent hover:text-[#1f2937]"
          >
            <X className="h-4 w-4" />
        ...
        <div className="flex max-h-[60dvh] flex-col gap-5 overflow-y-auto p-1">
          ...
          {postTitle && (
            <div className="flex flex-col gap-[3.5px] rounded-2xl border border-[#f3f4f6] bg-[#f8fafc] px-3 py-3">
              <p className="text-[12px] font-medium leading-[18px] text-[#99a1af]">
                Reporting Post:
              </p>
              <p className="text-[14px] font-normal leading-[21px] text-[#344256]">
                "{postTitle}"
              </p>
            </div>
          )}
          ...
          <div className="flex flex-col gap-3">
            <Label className="text-[14px] font-bold leading-[21px] text-[#344256]">
              ...
              {REPORT_REASONS.map((reason) => {
                ...
                return (
                  <button
                    ...
                    className={`h-[42px] w-full rounded-2xl border px-[14px] text-left text-[13px] font-medium leading-[19.5px] transition-colors ${
                      isSelected
