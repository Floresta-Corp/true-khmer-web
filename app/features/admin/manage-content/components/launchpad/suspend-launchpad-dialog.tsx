...
interface SuspendLaunchpadDialogProps {
  launchpadId: string;
  projectName: string;
  suspended: boolean;
  onSuspend: (launchpadId: string, reason: string) => void;
  onUnsuspend: (launchpadId: string) => void;
  disabled?: boolean;
  withLabel?: boolean;
  className?: string;
}

export default function SuspendLaunchpadDialog({
  launchpadId,
  projectName,
  suspended,
  onSuspend,
  ...
  const handleConfirm = () => {
    setOpen(false);
    if (suspended) onUnsuspend(launchpadId);
    else onSuspend(launchpadId, reason);
  };

  const actionLabel = suspended ? "Lift suspension" : "Suspend project";
  const ariaLabel = `${actionLabel}: ${projectName}`;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm rounded-2xl border border-slate-200 bg-white text-slate-900 shadow-xl dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:shadow-black/40">
        <DialogTitle>
          {suspended ? "Lift this suspension?" : "Suspend this project?"}
        </DialogTitle>
        <DialogDescription className="dark:text-slate-400">
          {suspended
            ? `“${projectName}” goes back to the status it had before the hold, and its poster is notified that it is public again.`
            : `“${projectName}” disappears from every listing, search result, and saved list except for its poster, who is notified along with the reason. Nobody can apply to it while it is held, and existing applications are left untouched. The hold is reversible.`}
        </DialogDescription>

        {!suspended && (
          <div className="space-y-2">
            <label
              htmlFor={`suspend-reason-${launchpadId}`}
