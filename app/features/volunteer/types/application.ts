...
export const BatchApplyApplicationInputSchema =
  ApplyApplicationInputSchema.omit({ roleId: true })
    .extend({
    ...
    .refine(
      (data) =>
        data.topPickRoleId === null ||
        data.roleIds.includes(data.topPickRoleId),
      { message: "topPickRoleId must be null or one of the selected roleIds" },
    );
export type BatchApplyApplicationInput = z.infer<
  typeof BatchApplyApplicationInputSchema
>;
