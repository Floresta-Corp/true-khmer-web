            ...
            <div className="mt-1 flex flex-col flex-wrap gap-1">
              {role.responsibilities
                ...
                .map((r, i) => (
                  <span
                    key={`resp-${i}`}
                    className="rounded-full px-2 py-0.5 text-xs text-[#2A4A7B]"
                  >
                    {r}
                  </span>
                ))}
              {role.responsibilities.filter((r) => safeTrim(r)).length > 3 && (
                <span className="rounded-full px-2 py-0.5 text-xs text-[#2A4A7B]">
                  +{role.responsibilities.filter((r) => safeTrim(r)).length - 3}
                </span>
              )}
            </div>
            ...
            <div className="mt-1 flex flex-col flex-wrap gap-1">
              {role.requirements
                ...
                .map((r, i) => (
                  <span
                    key={`req-${i}`}
                    className="rounded-full px-2 py-0.5 text-xs text-[#92400E]"
                  >
                    {r}
                  </span>
                ))}
              {role.requirements.filter((r) => safeTrim(r)).length > 3 && (
                <span className="rounded-full px-2 py-0.5 text-xs text-[#92400E]">
                  +{role.requirements.filter((r) => safeTrim(r)).length - 3}
                </span>
              )}
            </div>
