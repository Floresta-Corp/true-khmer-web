import { apiRequestWithOptionalSession, apiRequestWithSession, ProtectedApiError } from "~/lib/server/api-client.server";
import type { CreateVolunteerCategoryInput, CreateVolunteerCategoryResponse, GetVolunteerCategoriesResponse } from "~/features/volunteer/types/category";
...
export async function getPublicVolunteerCategories(request: Request) {
    try {
        const result = await apiRequestWithOptionalSession<GetVolunteerCategoriesResponse>(request, `/volunteer/public/categories`, {
            method: "GET",
        });
        return result;
    } catch (error) {
